...
from PseudoRegressorKeras import get_activation, get_inv_activation, RandomOrthogonal, reshape_input_matrix_to_ls_shape, reshape_ls_coefs_to_keras_weights, get_mod_boundary, reshape_ls_shape_to_output_matrix

class RandomPatch_ELM():
    ...
    def fit(self,trainX,trainY_raw):
        ...
        for layer_index in conv_layers:
            ...
            random_indices = np.random.choice(len(reshaped_activations), size=self.n_conv_filters, replace=False)
            random_patches = reshaped_activations[random_indices].T
            weights = reshape_ls_coefs_to_keras_weights(random_patches)
...
if __name__=="__main__":
    # Some Tests
    test_mat = np.random.uniform(-1 ,1, size=(10,9,9,10)).astype("float32")

    inp = Input(shape=test_mat[0].shape)
    x = Conv2D(4, kernel_size=(3,3), strides=(3,3))(inp)
    model = Model(inputs=inp, outputs=x)

    activation = test_mat[:, :get_mod_boundary(test_mat.shape[1]), :get_mod_boundary(test_mat.shape[2]), :]
    reshaped_activations = reshape_input_matrix_to_ls_shape(activation)
    random_indices = np.random.choice(len(reshaped_activations), size=4, replace=False)
    random_patches = reshaped_activations[random_indices].T
    weights = reshape_ls_coefs_to_keras_weights(random_patches)

    ls_output = reshaped_activations @ random_patches
    reshaped_ls_output = reshape_ls_shape_to_output_matrix(ls_output, model.output_shape[1:])

    bias = model.layers[1].bias.numpy()
    model.layers[1].set_weights([weights, bias])
    model_output = model(test_mat).numpy()

    if np.max(reshaped_ls_output - model_output) / np.min(reshaped_ls_output) > 1e-3:
        raise Exception("Random Patch Weight setting unit test failed")

    # Check if the resulting weights are indeed a patch => each patch must be represented squared in the outputs
    squared_patches = (random_patches.T @ random_patches).flatten()
    for squared_patch in squared_patches:
        if not np.any(model_output - squared_patch < 1e-6):
            raise Exception("Squared Patch not found in the outputs")